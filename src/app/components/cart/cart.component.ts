...
export class CartComponent implements OnInit {
  ...
  private renderDataCart(): void {
    this.dataService.renderCartData().subscribe(snapshot => {
  ...
  private getTotalPrice(): void {
    this.totalPrice = 0;
    this.cartData.forEach(item => (this.totalPrice += item["price"]));
  }

  public deleteDataItem(): void {
    this.dataService.deleteItemCart(this.itemId).then(res => {
      this.cartData = [];
      this.renderDataCart();
    });
  }
